  ...
  return (
    <footer className="mt-auto bg-[#1a3d1f] text-white">
      ...
      <div className="page-container py-8 sm:py-12">
        {/*
          Mobile:  logo row full-width, then 2-col grid (links + contact)
          Desktop: 4-col grid
        */}
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-10">

          {/* Logo + tagline + socials — spans 2 cols on mobile */}
          <div className="col-span-2 lg:col-span-1 space-y-3">
            <Link to="/" className="block w-fit">
              <div className="bg-white rounded-xl px-3 py-1.5 shadow-lg hover:opacity-90 transition-opacity inline-flex items-center">
                <img src={fullLogo} alt="TechzGreen" loading="lazy" className="h-10 sm:h-12 w-auto object-contain" />
              </div>
            </Link>
            <p className="text-[rgba(200,230,201,0.75)] text-xs sm:text-sm leading-relaxed max-w-xs">
              Transforming plastic waste into eco-friendly products through innovative recycling technologies.
            </p>
            <div className="flex items-center gap-2 flex-wrap">
              {SOCIALS.map(({ svg, title, href }) => (
                <a
                  key={title}
                  href={href}
                  target="_blank"
                  ...
                  aria-label={title}
                  className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-[rgba(255,255,255,0.08)] hover:bg-[rgba(76,175,80,0.3)] text-[rgba(200,230,201,0.8)] hover:text-white transition-all flex items-center justify-center"
                >
                  {svg}
                </a>
              ))}
          ...
          {/* Quick links */}
          <div>
            <h4 className="font-black text-xs uppercase tracking-widest text-[#4caf50] mb-3 sm:mb-4" style={{ fontFamily: 'Outfit, sans-serif' }}>
              Quick Links
            </h4>
            <ul className="space-y-2">
              {LINKS.map(({ label, to }) => (
                <li key={label}>
                  <Link to={to} className="text-[rgba(200,230,201,0.75)] hover:text-white text-xs sm:text-sm font-medium transition-colors">
                    {label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-2 text-xs sm:text-sm text-[rgba(200,230,201,0.75)]">
                <MapPin className="w-3.5 h-3.5 mt-0.5 text-[#4caf50] shrink-0" />
                <span>JJ Complex, Koonammoochi (P.O)<br />Thrissur, Kerala — 680504</span>
              </li>
              <li className="flex items-center gap-2 text-xs sm:text-sm">
                <Mail className="w-3.5 h-3.5 text-[#4caf50] shrink-0" />
                <a href="mailto:techzgreen23@gmail.com" className="text-[rgba(200,230,201,0.75)] hover:text-white transition-colors break-all">
                  techzgreen23@gmail.com
                </a>
              </li>
              <li className="flex items-center gap-2 text-xs sm:text-sm">
                <Phone className="w-3.5 h-3.5 text-[#4caf50] shrink-0" />
          ...
          {/* Mission — hidden on mobile, visible sm+ */}
          <div className="hidden sm:block">
            <h4 className="font-black text-xs uppercase tracking-widest text-[#4caf50] mb-3 sm:mb-4" style={{ fontFamily: 'Outfit, sans-serif' }}>
              Our Mission
            </h4>
            <p className="text-[rgba(200,230,201,0.75)] text-sm leading-relaxed mb-4">
              Building sustainable waste management solutions and promoting a circular economy through collection campaigns and environmental awareness programs.
            </p>
            <Link to="/signup" className="inline-flex items-center gap-2 btn-accent text-sm !py-2 !px-4">
              Join the Movement
            </Link>
          </div>
      ...
      <div className="border-t border-[rgba(255,255,255,0.08)]">
        <div className="page-container py-3 sm:py-4 flex flex-col sm:flex-row justify-between items-center gap-1 text-[10px] sm:text-xs text-[rgba(200,230,201,0.5)]">
